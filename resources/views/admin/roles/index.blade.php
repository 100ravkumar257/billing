	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				...
				<div class="card-body">
					<table class="table table-report text-center" id="role_table" >
						<thead>
							<tr>
						...
						<tbody>
							@foreach($roles as $role)
								<tr>
									<td>{{$role->id}}</td>
									<td>{{$role->name}}</td>
									<td>{{ $role->parent_name ?? 'No/parent' }}</td> 
									<td>{{ $role->status}}</td>
									<!-- <td>{{$role->code}}</td> -->
...
<script type="text/javascript">
	function changeRoleStatus(_this, id) {
		var status = $(_this).prop('checked') == true ? 1 : 0;
		let _token = $('meta[name="csrf-token"]').attr('content');

		$.ajax({
			url: `{{route('roles.status_update')}}`,
			type: 'get',
			data: {
				_token: _token,
				id: id,
