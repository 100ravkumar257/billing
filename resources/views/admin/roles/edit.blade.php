					...
					<div class="card-body">
						<div class="row">
							<div class="col-md-12">
								...
								<div class="form-group">
                                    <label for="parent_id" class="required">{{ __('parent_role') }}</label>
                                    <select name="parent_id" id="parent_id" class="select2">
                                        <option value="">{{ __('select_parent') }}</option>
                                        @foreach ($roles as $parentRole)
                                            <option value="{{ $parentRole->id }}" {{ $role->parent_id == $parentRole->id ? 'selected' : '' }}>
                                                {{ $parentRole->name }}
                                            </option>
                                        @endforeach
                                    </select>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" name="code" id="code"  placeholder="Enter code" value="{{ $role->code }}" required>
								...
								<div class="form-group">
                                        ...
                                        <select type="text" name="status" id="status" class="form-control @error('status') form-control-error @enderror" required="required">
                                            <option value="1" @if($role->status == 1) selected @endif>{{ __('active') }}</option>
                                            <option value="0" @if($role->status == 0) selected @endif>{{ __('inactive') }}</option>
                                        </select>
