				...
				@if (Gate::check('cmscategory-create'))
					<div class="col-md-3">
						<div class="create-btn pull-right">
							<a href="{{ route('categories.create') }}" class="btn custom-create-btn">{{ __('cmscategory.form.add-button') }}</a>
						</div>
					</div>
				@endif
			</div>
	...
	$(function() {
		$('#table').DataTable({
			...
			columns			: [
                    ...
                    if (data) {
                        return `<img src="{{ asset('storage/categary') }}/${data}" alt="${row.name}" width="50" height="50">`;
                    }
                    return 'No Image'; 
                }},
					{ data: 'name', name: 'name' },
...
<script type="text/javascript">
	function changeCmsCategoryStatus(_this, id) {
		...
		$.ajax({
			url: `{{route('categories.status_update')}}`,
			type: 'get',
			data: {
				_token: _token,
				id: id,
