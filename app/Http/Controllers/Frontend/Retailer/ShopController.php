...
class ShopController extends Controller
{
    ...
    public function index()
    {
        ...
        $productsQuery = Product::with(['variants' => function ($query) {
            $query->select('product_id', 'size');
        }])
        ...
        if ($brandids && !empty($brandids[0])) {
            $brandIdsArray = explode(',', $brandids[0]); 
            $brandIdsArray[] = $brand_id; 
            $productsQuery->whereIn('brand_id', $brandIdsArray);
        }
    
        $products = $productsQuery->get();
        $activeCategorySlug = $category ? $category->slug : ''; 
